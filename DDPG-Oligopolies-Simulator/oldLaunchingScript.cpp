//
//  oldLaunchingScript.cpp
//  DDPG-Oligopolies-Simulator
//
//  Created by Aldric Labarthe on 05/03/2024.
//
...
// Currently, we test the adjunction of memory
LearningAgent::gamma = 0.9;

std::size_t maxiterations(60000);
// We test with more it
//std::size_t maxiterations(90000);
LogManager.reserve(2001);
struct episode {
    float agent1Action;
    float agent2Action;
...
for (std::size_t iteration(0); iteration <= maxiterations/128; iteration++){
    ...
    for (std::size_t inside_iteration(0); inside_iteration < 128; inside_iteration++){
        ...
        //else
            //whitenoise = ((it % 10 == 0) ? 0.f : 0.3 * (1.f-(float)it/(float)maxiterations));
            whitenoise = ((it % 10 == 0) ? 0.f : 0.3 * (1.f - std::powf(1.0001, (float)((float)it -(float)maxiterations))));
